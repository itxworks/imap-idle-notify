...
// --- real notification servers ---------------------------------------------
//
// Unlike the recorder (which only captures the wire bytes), these start the
// actual Gotify / ntfy servers in containers so the tests prove the daemon's
// request is accepted by a real implementation and the message it produced can
// be read back through the server's own API.
